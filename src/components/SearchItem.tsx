...
const SearchItem: FC<Props> = ({ search, modalOpen, setModalOpen,loading}) => {
  ...
  return (
          <div onClick={() => { imgLoaded?"":close()}} className={`flex items-center p-2 max-w-full rounded-lg m-2 cursor-pointer`} style={{ background: "linear-gradient(45deg, black, transparent)" }}>
            ...
            <div className='w-full'>
              <div><h6>{imgLoaded?<WavesLoader height="10px"/>:search?.artist_names}</h6></div>
              <p className='text-2xl flex items-center justify-center'>{imgLoaded?<div className='flex mx-auto'><WavesLoader height="10px"/><WavesLoader height="10px"/><WavesLoader height="10px"/><WavesLoader height="10px"/></div>:search?.full_title}</p>
            </div>
          </div>
